...
private:
	...
	void buildPaths(string inputDirectory, string outputDirectory, vector<string>& directoryNames)
	{
		for (int i = 0; i < directoryNames.size(); i++) {
			string resultDirectoryName = outputDirectory + "\\" + directoryNames.at(i);
			create_directory(resultDirectoryName);

			string directoryName = inputDirectory + "\\" + directoryNames.at(i);
			for (directory_iterator itr(directoryName); itr != directory_iterator(); ++itr) {
				path classPath = itr->path();
				if (is_directory(classPath)) {
					create_directory(resultDirectoryName + "\\" + classPath.filename().string());

					string imageDirectoryName = classPath.parent_path().string() + "\\" + classPath.filename().string();
